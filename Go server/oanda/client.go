...
func NewFxPracticeClient(token string, r *http.Request) (*Client, error) {
	if token == "" {
		return nil, errors.New("No FxPractice access token")
	}
	return NewClient("fxpractice", token, urlfetch.Client(appengine.NewContext(r)))
}
...
//
// See http://developer.oanda.com/docs/v1/auth/ for further information.
func NewFxTradeClient(token string, r *http.Request) (*Client, error) {
	if token == "" {
		return nil, errors.New("No FxTrade access token")
	}
	return NewClient("fxtrade", token, urlfetch.Client(appengine.NewContext(r)))
}
